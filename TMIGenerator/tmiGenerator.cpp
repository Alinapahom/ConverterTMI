...
size_t TMIGenerator::generateTMI(void *byteBuffer, size_t bufferSize)
{
    ...
    tmiStruct->marker = __bswapd(TMI_MARKER);   // Копирование маркера пакета
    ...
    std::cout << tmiStruct->dstIP << "DEBUG test\r\n";
    ...
    auto dataPolePtr = &(tmiStruct->minSizeData);         // Получение адреса начала блока данных
...
{
    char myHostName[256];                                       // Определение структуры с именем хоста
    if (gethostname(myHostName, 256) == -1)                     // Получение имени хоста и проверка на ошибки
        return 0x7F000001;                                      // Возврат адрес локального хоста (127.0.0.1), с случае ошибки
    auto host_entry = gethostbyname(myHostName);                // Получение адреса структуры IP адреса
